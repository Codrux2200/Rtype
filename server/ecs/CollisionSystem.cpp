    ...
    void CollisionSystem::update(SceneManager &sceneManager, float deltaTime, std::vector<Network::Packet> &packetQueue)
    {
        for (auto entity : sceneManager.getCurrentScene()->entitiesList) {
            ...
            for (auto entity2 : sceneManager.getCurrentScene()->entitiesList) {
                ...
                float x1 = pos1[0] + hitbox1[0].first;
                float x2 = pos1[0] + hitbox1[1].first;
                float y1 = pos1[1] + hitbox1[0].second;
                float y2 = pos1[1] + hitbox1[1].second;

                float _x1 = pos2[0] + hitbox2[0].first;
                float _x2 = pos2[0] + hitbox2[1].first;
                float _y1 = pos2[1] + hitbox2[0].second;
                float _y2 = pos2[1] + hitbox2[1].second;

                if ((_x1 > x1 && _x1 < x2 && _y1 > y1 && _y1 < y2) ||
                    (_x2 > x1 && _x2 < x2 && _y1 > y1 && _y1 < y2) ||
                    (_x1 > x1 && _x1 < x2 && _y2 > y1 && _y2 < y2) ||
                    (_x2 > x1 && _x2 < x2 && _y2 > y1 && _y2 < y2)) {
                    hitboxComponent->executeCallback(entity2);
                    hitboxComponent2->executeCallback(entity);
                }
            }
        }
    }
